...
const PATTERNS: Record<LatticePatternName, Partial<Record<LatticeGrid, ResolvedPattern>>> = {
  ...
  pulse: {
    4: { cells: [2, 1, 1, 2, 1, 0, 0, 1, 1, 0, 0, 1, 2, 1, 1, 2], loop: 2.4, scale: 2.5, lit: 0.45 },
  },
};
...
export const LatticeLoader: React.FC<LatticeLoaderProps> = ({
  ...
  const [finalAnnounce, setFinalAnnounce] = useState("");
  const announce = status === "working" ? `${label}, in progress` : finalAnnounce;
  ...
  useEffect(() => {
    // Only reacts to a done/error transition: captures the elapsed timer ref (an
    // external, imperatively-updated value) and latches the mark so it stays visible
    // if status later flips back to "working".
    if (status === "working") return;
    /* eslint-disable react-hooks/set-state-in-effect -- captures dsRef (an imperative
       timer, not render state) at the instant status transitions. */
    setLastMark(status);
    setFinalAnnounce(
